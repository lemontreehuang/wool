...
function MystUserInfo:mystUseSpeItem(player, slot)
	...
	if mystSpeInfo and mystSpeInfo[sourceItemID] then
		speRideID = mystSpeInfo[sourceItemID]
	end

	if speRideID <= 0 then
		local speItemInfo = g_mystShopMgr:getMystItemInfo(MYSTERYSHOP_BLACK, 1, sourceItemID)
		if speItemInfo.spe then
			speRideID = speItemInfo.spe
			mystSpeInfo[sourceItemID] = speItemInfo.spe
	...
	if ret then
		...
		g_logManager:writePropChange(roleSID,2,233,sourceItemID,0,1,0)
	end
end
